...
variable "github_oidc_thumbprint" {
  description = "Thumbprint for the GitHub OIDC provider certificate chain (set per AWS region guidance)."
  type        = string
  default     = "replace-with-github-oidc-thumbprint"

  validation {
    condition     = length(trim(var.github_oidc_thumbprint)) > 0
    error_message = "github_oidc_thumbprint must be provided."
  }
}
